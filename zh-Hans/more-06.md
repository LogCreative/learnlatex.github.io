...
`\usepackage` 命令接受宏包逗号分隔列表作为参数，所以您可以在一行中加载多个宏包：比如，`\usepackage{color,graphicx}`。如果您向其传递参数，那么它们就会向列表中所有的宏包传递。因为如果分别加载的话，注释掉宏包也会更简单，所以我们会保持一行加载一个宏包这个习惯。

## `babel` 宏包

在 [主课](lesson-06) 中我们展示了用来选择不同断字模式的 `babel` 宏包。取决于使用的不同语言，它可能不仅只做了这么断字一件事。比如，在德语中，它不仅提供了创建“软”断字符的一些简写，还提供了一种不需要德式键盘来输入元音变音的快速方法。以及可以注意到，一般使用 `\tableofcontents` 生成的 _目录_ 节标题已经变成了德语 Inhaltsverzeichnis。
...
有些时候，您想要将一个选项传递给加载的所有宏包。这可以通过在 `\documentclass` 这一行给出选项实现：每一个宏包都可以“看到”这个选项列表。所以为了将语言选项传递给所有宏包，我们可以使用：
...
和 `\newcommand` 只需要指定含有多少个参数不同，`\NewDocumentCommand` 可选地提供第一个参数的默认值之后，每个参数都会被一个字母指定，所以一个含有两个参数的命令会被指定为 `{mm}` 而不是 `[2]`。虽然这可能会繁冗一些，但这样可以允许更多的命令形式被定义。这里我们仅给出了一个简单的例子，第一个参数可选，默认为 blue（`O{blue}`），第二个参数必需（`m`，mandatory）。
