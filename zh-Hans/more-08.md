...
通常 LaTeX 对列的两边加些间距，以产生平衡视觉并分隔它们。这个间距通过 `\tabcolsep` 定义。由于每一列都会在两边增加一个 `\tabcolsep` 的间距，所以两列间的间距是 `2\tabcolsep` —— 每列一个单位长。您可以通过 `\setlength` 将这个间距和改变到任何长度：
...
您可以通过使用 `@` 来将这个间距变更为一任意长度。这将移除两列间的空隙或者是某一端的空隙，取而代之的是在参数中添加任何放在两列间的东西。
