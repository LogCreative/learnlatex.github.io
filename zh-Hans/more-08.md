...
`\itshape` 将所有后续的文字变为斜体，但效果被限制在单元格的“容器”中。我们将会在[后续课程](lesson-11)中学习手动设定文本格式。
...
通常 LaTeX 对列的两边加些间距，以产生平衡视觉并分隔它们。这个间距通过 `\tabcolsep` 定义。由于每一列都会在两边增加一个 `\tabcolsep` 的间距，所以两列间的间距是 `2\tabcolsep` —— 每列一个单位长。您可以通过 `\setlength` 将这个间距改变到任何长度：
...
您也可以通过使用 `@` 来将这个间距变更为一任意长度。这将移除两列间的空隙或者是某一端的空隙，取而代之的是在参数中添加任何放在两列间的东西。
...
（我们将会[很快看到](lesson-11) `\hspace`：可以猜到它增加了水平间距。）
...
所有的 `booktabs` 分割线和 `\addlinespace` 支持一个方括号内的可选参数用来指定分割线的宽度。另外，`\cmidrule` 通过在 `r` 或 `l` 后的花括号中指定长度可以自定义缩短范围。
...
注意，大部分情况下推荐使用下面的方式格式化表格到指定的宽度（必要时或许使用例如 `\small` 的文字大小），而不是将表格使用 `\resizebox` 或相似的命令缩放，因为这会产生不一致的文字大小和分割线宽度。
...
`tabularx` 环境（被同名宏包提供）含有和 `tabular*` 相似的名称，但是相较于调整列间间距，它通过指定一种新的列类型 `X` 来调整列宽。这等价于自动确定宽度的 `p{...}` 类型。
...
好几个宏包提供了相似名称的变种以允许跨页。这里我们展示 `longtable` 宏包：
...
`longtable` 很重要因为它保持了所有页的表格列宽度。然而，为了实现这一点，它可能需要 LaTeX 的多次运行以使得后面加载的宽单元格可以影响之前页中的表格宽度。

## 表格标注

表格中类似于脚注的标记用来代指表格后面的标注是很常见的需求。`threeparttable` 宏包简化了这种表格的标记方法，将标注所在区块的宽度设定为表格同宽。详细细节参见[宏包文档](https://texdoc.org/pkg/threeparttable)，但是这里我们展示一个简单的例子。
...
默认的断行设置假定行宽相对较大，以在断行选择上给予一些灵活性。下面的例子展示了一些可能的方法。第一个表格展示了扩张的词间间距，TeX 会警告之为未填满的行盒子。使用 `\raggedright` 通常避免了这个问题，但也使得一些行太“锯齿”了。`ragged2e` 宏包的 `\RaggedRight` 命令是一种折中方案——它将允许行宽上的一定的锯齿形态，但也会在必要时断词，正如第三个表格所展示的那样。
...
正如[主课](lesson-08)所展示的那样，`array` 宏包允许构造诸如 `>{\bfseries}c` 以表示一个粗体居中列。定义一个新的列类型来封装这种用法通常是方便的，比如
...
在主课中我们演示了 `booktabs` 宏包中的 `\addlinespace` 用来在指定行间增加额外的间隙。

主要有两个参数用以控制行间距：`\arraystretch` 和 `\extrarowheight`（后者来自 `array` 宏包）。
