...
在定义中，`[1]` 表示参数个数（这里只有一个），`#1` 表示提供的第一个参数（在这个例子中，指的是 `apples` 或 `oranges`）。您最多可以设置 9 个参数，但一般情况下最好只有一个参数，有时甚至不需要设置任何参数。

在排版一篇文档的过程中，定义命令的作用不仅在于减少输入量，还有助于将格式信息分离出来。如果对于关键词准备采用不同的格式，您只需要调换成不同的定义即可，而不是对整篇文档从头到尾地编辑一遍。这里为了设定彩色加载了 `xcolor` 宏包，在格式上把加粗改成了蓝色。
...
尝试使用其他欧洲语言撰写一些文段，并观察 `babel` 是怎么影响断字的：您可以从网上搜集一些文段，并猜测正确的设置应该是什么。
