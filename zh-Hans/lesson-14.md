...
在 Unicode 出现之前，LaTeX 提供了许多种**文件编码**来允许很多语言的文字以原生的方式输入——比如：使用 `latin1` 编码，法国用户就可以输入「`déjà vu`」，LaTeX 会将重音字母在内部转译为 TeX 命令以产生正确的输出。 

这种方法至今仍在 LaTeX 的 `pdflatex` 引擎中使用。除非另有说明，默认情况下所有的文件都被认为是 Unicode（以 UTF-8 编码）的。纵使这个引擎会限制字体为 8 位长度的，许多欧洲语言仍可被支持。
...
[CTAN 中的「字体」主题](https://www.ctan.org/topic/font)以查阅一些选择。你也可以在网上搜索需要的字体，并寻找一个 `pdflatex` 兼容的版本。如果你想要使用一种专有字体，可以尝试搜寻一个合适的副本（大部分情况下都存在与原版足够类似的那种）。
...
选择好字体之后，就可以直接向文档里输入普通的 Unicode 文本了。下面的例子展示了一些拉丁字母、德语字母以及中日韩（CJK）表意文字的输入：
...
<p 
  class="hint">切换语言的同时，改变断字方式等相关事项通常也是很重要的。<code>babel</code> 和 <code>polyglossia</code> 宏包为此提供了稳健的功能。</p>
