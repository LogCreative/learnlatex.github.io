...
LaTeX 不仅提供了专注于文档逻辑结构的方式，也提供了直接修改外观的方式。大部分情况下，最好使用前者，因为前者在迫不得已的时候可以轻易地复用或更改外观。

## 结构与视觉呈现

我们将从一个例子出发，展现 LaTeX 中最常见的逻辑标记命令之一——`\emph`：将文字变成斜体。（印刷界里，为了强调文字，经常会这么做。）
...
或许你能够猜得到 `\textit` 让文字变成斜体，但是也**只**是变成斜体，不能嵌套使用，并且可以看到 `\emph` 能够嵌套使用。当然，也有强调格式不是斜体的情况：比如在幻灯片中，使用颜色来强调会更好。在使用逻辑标记的方法之后，我们就可以不用担心编写文档主体时格式设定上的细节问题。

我们会稍后看到[手动设定格式](lesson-11)的相关内容，现在我们为这一段添加一个已经学过的 `\textbf` 命令：让文字变粗的命令。

## 目次命令

或许你使用过字处理器：为了开始一节，多数人的习惯是输入标题文字，然后简单地让它变大、变粗，换行继续。在 LaTeX 中，使用逻辑标记的方式比这种手动设定格式的方式**容易**得多——使用 `\section` 命令：它不仅可以直接控制字体变化、纵向间距……，还可以让整个文章的格式统一。
...
还有一种使用逻辑标记的很常见情况就是撰写列表。LaTeX 主要内置两种常见的列表类型。
...
制作一些列表，并在一个列表中嵌套使用另一个列表。编号和标记的格式是怎么变化的？你至多可以在标准的 LaTeX 中嵌套四层，毕竟多于四层的嵌套列表一般并不是一个很好的信号！