...
`\input` 命令适合于 _不_ 独立于章节的长文件片段。例如，我们用它来分隔前封面和后封面，来保持主文件的简短清晰，并可以在其他文档中复用。我们也用这个命令来分割书的起始“非章节”的目次：比如前言。这也是为了保持主文件的结构清晰。
...
添加一些浮动体并输出图目录和表目录。如果使用本地 LaTeX 编译，您能看出来 LaTeX 编译了几次吗？（在线版本会在后台自动重复运行 LaTeX，所以额外的编译次数看起来可能并没有那么明显。）
